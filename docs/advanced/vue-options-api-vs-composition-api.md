...
- **补充经验——团队需要风格约定**：大团队里"想放哪放哪"会导致质量参差，建议配套 style guide、评审规范，甚至借助 code mod 做迁移辅助（Evan 也提到 code mod 很难做到"一刀切"，但 80% 的自动化方案或许可行）。
- **补充经验——关注 Vapor 与生态走向**：Vapor 模式仅支持 Composition API，越来越多库也只支持它。即便当下用 Options API 没问题，长期看熟悉 Composition API 是更稳的投资。