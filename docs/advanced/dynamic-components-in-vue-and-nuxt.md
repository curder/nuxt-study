...
- **别滥用全局注册**：全局组件会常驻应用实例，数量膨胀会拖慢启动并增加内存占用。除非确实跨页面高频复用或 CMS 场景，否则局部 + `Lazy` 通常是更可控的默认选择。
- **给动态组件加兜底**：CMS 返回的组件名可能不存在，建议在 `computed` 里对未知名称做降级处理（如渲染占位或 fallback 组件），避免线上出现空白。